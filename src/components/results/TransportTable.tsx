...
export function TransportTable({ snapshot }: TransportTableProps) {
  ...
  const selectedKey = selectedCell
    ? cellKey(selectedCell[0], selectedCell[1])
    : null;
  ...
  function getCellBg(i: number, j: number): string {
    ...
    if (isSelected)
      return "bg-[#F2F2F2] outline outline-2 outline-black outline-offset-[-2px]";
    if (inCycle)
      return "bg-[#F2F2F2] outline outline-2 outline-black outline-offset-[-2px] outline-dashed";
    if (isBasic) return "bg-[#F2F2F2]";
    return "bg-white";
  }

  const thBase =
    "py-3 px-2 text-center text-[14px] font-medium border-b-2 border-black";
  const tdBase = "py-3 px-2 text-center border-b border-black relative";
  const labelTd =
    "py-3 px-2 text-left text-[14px] font-medium border-b border-black";
      ...
      <table className="w-full border-collapse border-t border-black">
        ...
        <tbody>
          {Array.from({ length: rows }, (_, i) => (
            <tr key={i}>
              <td
                className={`${labelTd} ${isFictitious.rows[i] ? "italic text-black/50" : ""}`}
              >
              ...
              {Array.from({ length: cols }, (_, j) => {
                ...
                return (
                  <td key={j} className={`${tdBase} ${getCellBg(i, j)}`}>
                    {isBlocked ? (
                      <span className="text-black/25 text-[18px] font-light select-none">
                        ×
                      </span>
                    ) : (
                      <div className="flex flex-col items-center gap-0.5">
                        ...
                        {cycleSign && (
                          <span className="absolute top-1 right-1.5 text-[11px] font-bold text-black leading-none">
                            {cycleSign}
                          </span>
                        )}
                      </div>
          ...
          {!isInitialPhase && (
            <tr>
              ...
              {Array.from({ length: cols }, (_, j) => (
                <td
                  key={j}
                  className={`${tdBase} font-mono text-[14px] text-black/60`}
                >
